...
def diamond_iteration_of_first_X_nodes(G, S, X, alpha):
    ...
    ########----Modifying Universe Size for DIABLE----------------######
    seed_region = { your_key: neighbors[your_key] for your_key in neighbors if your_key in list(cluster_nodes) }
    N_resized = set(seed_region.keys())
    
    for key in seed_region:
        N_resized.update(seed_region[key])
        
    new_N_resized = N_resized.difference(cluster_nodes)
    
    second_seed_region = { your_key : neighbors[your_key] for your_key in neighbors if your_key in list(new_N_resized) }
    
    for key in second_seed_region:
        new_N_resized.update(second_seed_region[key])
    
    N = len(set.union(new_N_resized, cluster_nodes))
...
if __name__ == '__main__':
    ...
    # run DIAMOnD
    new_max_number_of_added_nodes = 50
    added_nodes = DIABLE(G_original,
                          seed_genes,
                          new_max_number_of_added_nodes, alpha,
